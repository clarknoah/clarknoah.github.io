...
- Direction: **refined-technical, cool**. A blueprint palette (cool near-white paper, ink text,
  deep architect's blue accent). Deliberately NOT the warm-cream + amber look, which read as
  Claude's own palette.
- Light is the default theme; dark + system available via the toggle.
- Type: two voices. **Newsreader** (editorial serif) is the human voice for all prose,
...
- Accent: deep architect's blue (`--color-accent`, #27457d light / #6f9ae0 dark), used as signal
  only (active, hover, focus, CTA, terminal cursor), never as decoration or a section-label
  colour. No warm cream or amber, no gradients, no glow shadows.
- Layout: editorial. Asymmetric grids, hairline definition-list rows over identical card grids,
  generous whitespace. One blue CTA + a quiet text link, never twin buttons. Gentle motion
